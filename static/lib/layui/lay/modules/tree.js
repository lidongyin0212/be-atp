...
layui.define('form', function (exports) {
    ...
    Class.prototype.config = {
        ...
        , limitNodeCopyLevel: 0   // 设置第x级节点不允许添加操作
        , limitNodeDelLevel: 0   // 设置第x级节点不允许删除操作
        , limitNodeUpdateLevel: 0
        , onlyNodeUpdateLevel: 0
        , uiNodeOperate: 0     // UI二级操作节点
        , customOperate: false    // 节点操作是否支持自定义操作
        , text: {
        ...
        layui.each(data, function (index, item) {
            var hasChild = item.children && item.children.length > 0
                ...
                entryDiv = $(['<div data-id="' + item.id + '" class="layui-tree-set' + (item.spread ? " layui-tree-spread" : "") + (item.checked ? " layui-tree-checkedFirst" : "") + '">'
                    ...
                    //复选框
                    , function () {
                        return options.showCheckbox ? '<input type="checkbox" case_level="' + (item.level) + '" name="' + (item.field || ('layuiTreeCheck_' + item.id)) + '" same="layuiTreeCheck" lay-skin="primary" ' + (item.disabled ? "disabled" : "") + ' value="' + item.id + '">' : '';
                    }()
                    ...
                    , function () {
                        if (options.isJump && item.href) {
                            ...
                            return '<span data_id="' + (item.id) + '" case_level="' + (item.level) + '" module_type="' + (item.module_type) + '"class="' + ELEM_TEXT + (item.disabled ? ' ' + DISABLED : '') + '">' + (item.title || item.label || options.text.defaultNodeName) + '</span>';
                        }
                    }()
                    , '</div>'
                    ...
                    , function () {
                        ...
                        var editIcon = {
                            run: '<i class="layui-icon" data-type="run" title="执行">&#xe601;</i>'
                            , add: '<i class="layui-icon layui-icon-add-1"  data-type="add" title="添加"></i>'
                            ...
                            , record: '<i class="layui-icon" data-type="record" title="记录">&#xe60e;</i>'


                        }, arr = ['<div class="layui-btn-group layui-tree-btnGroup">'];
                        // 修改源码
                        if (options.limitNodeCopyLevel > 2 && item.level >= options.limitNodeCopyLevel) {
                            editIcon = {
                                copy: '<i class="layui-icon layui-icon-file-b" data-type="copy" title="复制"></i>'
                                , del: '<i class="layui-icon layui-icon-delete" data-type="del" title="删除"></i>'
                            }
                        }
                        if (options.limitNodeDelLevel >= 0 && item.level == options.limitNodeDelLevel) {
                            editIcon = {
                                add: '<i class="layui-icon layui-icon-add-1"  data-type="add" title="添加"></i>',
                                run: '<i class="layui-icon" data-type="run" title="执行">&#xe601;</i>',
                                record: '<i class="layui-icon" data-type="record" title="记录">&#xe60e;</i>'
                        ...
                        if (options.limitNodeUpdateLevel > 0 && item.level == options.limitNodeUpdateLevel) {
                            editIcon = {
                                add: '<i class="layui-icon layui-icon-add-1"  data-type="add" title="添加"></i>',
                                update: '<i class="layui-icon layui-icon-edit" data-type="update" title="修改"></i>'
                            }
                            if (item.title != "前置条件" && item.title != "后置条件"){
                                editIcon.del = '<i class="layui-icon layui-icon-delete" data-type="del" title="删除"></i>'
                            }
                        }
                        if (options.uiNodeOperate > 0 && item.level >= options.uiNodeOperate) {
                            editIcon = {
                                run: '<i class="layui-icon" data-type="run" title="执行">&#xe601;</i>'
                                , del: '<i class="layui-icon layui-icon-delete" data-type="del" title="删除"></i>'
                                , copy: '<i class="layui-icon layui-icon-file-b" data-type="copy" title="复制"></i>'
                        ...
                        if (options.onlyNodeUpdateLevel >= 0 && item.level == options.onlyNodeUpdateLevel) {
                            editIcon = {
                                update: '<i class="layui-icon layui-icon-edit" data-type="update" title="修改"></i>'
                            }
                        }
    ...
    Class.prototype.spread = function (elem, item) {
        ...
        elemText.on('click', function () {
            var othis = $(this);
            // 添加源码
            layui.$('.' + ELEM_TEXT).removeClass('custom-tree-item-clicked');
            elemText.addClass('custom-tree-item-clicked');
            //判断是否禁用状态

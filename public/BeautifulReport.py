...
class PATH:
    ...
    # config_tmp_path = TEMPLATE_PATH + '/template/template'
    config_tmp_path = TEMPLATE_PATH + '/public/template/template'
...
class ReportTestResult(unittest.TestResult):
    ...
    def stopTest(self, test) -> None:
        ...
        result = self.get_all_result_info_tuple(test)
        # if self.type not in ['2', '3']:
    ...
    def stopTestRun(self, title=None) -> dict:
        ...
        :return:
        """
        self.FIELD['testPass'] = self.success_counter
        for item in self.result_list:
            item = json.loads(str(MakeResultJson(item)))
            self.FIELD.get('testResult').append(item)
        self.FIELD['testAll'] = len(self.result_list) - len(self.test_other)
        self.FIELD['testName'] = title if title else self.default_report_name
        self.FIELD['testFail'] = self.failure_count
        self.FIELD['beginTime'] = self.begin_time
        end_time = int(time.time())
        start_time = int(time.mktime(time.strptime(self.begin_time, '%Y-%m-%d %H:%M:%S')))
        self.FIELD['totalTime'] = str(end_time - start_time) + 's'
        self.FIELD['testError'] = self.error_count
        self.FIELD['testSkip'] = self.skipped
        self.FIELD['task_id'] = self.task_id
        self.FIELD['interface_count'] = len(set(self.interface_id_list))
        self.FIELD['all_interface_count'] = int(self.interface_count)
        self.FIELD['interface_coverage'] = '{0:.3}%'.format(
            str((len(set(self.interface_id_list)) / int(self.interface_count)) * 100))
    ...
    def get_all_result_info_tuple(self, test) -> tuple:
        ...
        :return:
        """
        self.interface_count = test.__dict__['_interface_count']
        if self.type in ['2', '3']:
            self.test_other.append(1)
        if self.type not in ['2', '3']:
            self.interface_id_list.append(test.__dict__['_interface_id'])

        self.case_log = test.__dict__['_log']
        self.task_id = test.__dict__['_task_id']
        return tuple([*self.get_testcase_property(test), self.end_time, self.status, self.case_log, self.type])
    ...
    def get_testcase_property(test) -> tuple:
        ...
        """
        class_name = test.__dict__['_testMethodDoc']
        method_doc = test.__class__.__qualname__
        req_path = test.__dict__['_req_path']
        interface_id = test.__dict__['_interface_id']
        return class_name, method_doc, req_path, int(interface_id)
...
class BeautifulReport(ReportTestResult, PATH):
    ...
    def report(self, description, filename: str = None, log_path='.'):
        ...
        print(text)
        return self.result_list, self.FIELD
